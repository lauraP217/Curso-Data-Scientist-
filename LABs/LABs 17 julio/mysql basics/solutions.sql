...
SELECT account_id,amount  FROM loan ORDER BY  loan_id LIMIT 5;
...
#Consulta 13: En la tabla client, de todos los distritos con un district_id menor que 10, ¿cuántos clientes hay de cada district_id? Muestra los resultados ordenados por district_id en orden ascendent
SELECT district_id,count(client_id) FROM client WHERE district_id<10 GROUP BY district_id ORDER BY district_id LIMIT 10;
...
SELECT account_id, type, SUM(amount) FROM trans WHERE account_id=396 AND type IN ('VYDAJ','PRIJEM') GROUP BY account_id, type ORDER BY type;
